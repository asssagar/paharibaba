        ...
        public void Download(string url, string destFolder)
        {
            ...
            try
            {
               ...
               // destFolder = destFolder.Replace("file:///", "").Replace("file://", "");
                if (!Directory.Exists("F://ClientContent"))
                {
                    Directory.CreateDirectory("F://ClientContent");
                    destFolder = "F://ClientContent";
                }
                else
                {
                    destFolder = "F://ClientContent";
                ...
                {
                    int fileapd = 1;
                    while (File.Exists(downloadingTo))
                    {
                        fileapd++;
                        destFileName = Path.GetFileNameWithoutExtension(downloadingTo) + fileapd.ToString() + Path.GetExtension(destFileName);
